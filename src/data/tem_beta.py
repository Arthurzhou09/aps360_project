...
class Tem1BetaLactamaseDataset(DataClass):
    ...
    def __init__(self, dms_data: pd.DataFrame, pdb_id:str, experiment_wt_sequence: str, directed = True, max_neighbours=None):
        ...
        wt_sequence, aligned_experimental_seq, valid_residue_mask= align_sequence(wt_sequence, experiment_wt_sequence, pdb_id) 
        valid_residue_indices = np.where(valid_residue_mask == True)[0]

        self.starting_index = valid_residue_indices[0]  # starting index of valid residues in the PDB sequence - usse to modfiy code so that positoin match with PDB WT
        self.dms = dms_data.loc[dms_data['Ambler Position'].isin(valid_residue_indices)].reset_index(drop=True)  # filter DMS data to only include valid residues
    ...
    def __getitem__(self, idx):
        ...
        code = self.dms.iloc[idx]['Code'].split("_")
        print(code)
        code = "_".join([str(int(i) - self.starting_index) if i.isnumeric() else i for i in code])  # adjust the ambler position to match the PDB WT sequence
        print(code)
        # work around: instead of using the normalized ambler position, we will use the index to match with the PDB WT seq. 
        ...
        distance_edge_features = self.distance_features

        mutation_residue_index = np.zeros(len(self.wt_sequence_encoded), dtype=bool)
        for pos in mutation_position:
            mutation_residue_index[pos] = True
    
        protein_graph = ProteinGraphData(
            distance_features=torch.tensor(distance_edge_features, dtype=torch.float),
